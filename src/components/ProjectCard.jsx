...
const ProjectCard = ({project}) => {
  return (
    <div className="relative group overflow-hidden h-full w-full rounded-xl border border-blue-bell-800 shadow hover:shadow-lg duration-300">
      <div className="absolute grain w-full h-full opacity-15 z-0" />
      <div className="relative overflow-hidden flex flex-col h-full z-10 recent-project-bg">
        <img
        ...
        <div className="p-4">
          <div>
            ...
            <p className="text-sm text-blue-bell-800">{project.type?.join(", ")}</p>
          </div>
          <p className="my-8 victor-mono text-sm text-pretty">{project.description}</p>
        </div>

        <div className="flex gap-2 mt-auto px-4">
          {project.tags.techstacks.map((t, index) => <IconFont key={index} name={t} size={20} tooltip={t} className="text-2xl" />)}
        </div>
        <div className="p-2">
